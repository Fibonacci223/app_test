...
def run_matrix_effect(master_app):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and now - start_time >= SHOW_INPUT_AFTER:
                input_active = True
        ...
        if now - start_time >= SHOW_INPUT_AFTER:
            ...
            input_alpha = min(input_alpha + 20, 255)
            ...
            s.fill((0, 120, 0, int(input_alpha * (0.5 + 0.5 * abs(1 - (now * 2) % 2)))))
            screen.blit(s, (input_box.x - 6, input_box.y - 6))
            pygame.draw.rect(screen, (0, 255, 0), input_box, 1)
            ...
            if error_timer > now:
                ...
                screen.blit(shadow_surface, (err_x + random.randint(-4, 4), label_y + random.randint(-4, 4)))
                ...
                for i in range(0, err_height, block_height):
                    ...
                    screen.blit(err_surface, (err_x + glitch_x, label_y + i + glitch_y), block_rect)

                glitch_rect_color = (random.randint(150, 255), 0, 0)
                pygame.draw.rect(screen, glitch_rect_color, input_box.move(random.randint(-5, 5), random.randint(-5, 5)), 2)
            ...
            if error_timer <= now:
                ...
                screen.blit(password_surface, (text_x, text_y))
                ...
                if cursor_visible:
                    ...
                    pygame.draw.line(screen, (0, 255, 0), cursor_pos, (cursor_pos[0], cursor_pos[1] + input_box_height - 24), 2)
